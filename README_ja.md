...
│ ashigaru3.yaml に    │──変更────▶│ (fswatch、ポーリング     │
...
|----------|------|-------------|---------------|
| 1番 | **自己監視** | エージェントが自分のinboxファイルを監視 — 自力で起床、nudge不要 | エージェント自身が `fswatch` を実行中 |
...
- **待機中のCPU使用率ゼロ** — `fswatch` はファイルシステムイベントでブロック（ポーリングループではない）。メッセージ間のCPUは0%。
...
| ポーリングでAPIコールを浪費 | `fswatch` はイベント駆動（待機中CPU 0%） |
...
│   ├── inbox_watcher.sh      # fswatchでinbox変更を監視
